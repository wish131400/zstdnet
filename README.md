...
ZstdNet 是一个 Minecraft Java 版模组，用 ZSTD 对客户端与服务端之间的流量做压缩转发，目标是降低高重复数据场景下的公网带宽占用，尤其适合机械动力类、大型整合包类服务器。
...
### 实际效果示例（服务器）
在齿轮盛宴（https://www.xn--dctt54dhmrbwo.com/）整合包中，
...
- `/zstdport show`：查看当前 Zstd 端口和游戏端口
- `/zstdport game <端口>`：设置后端游戏端口
- `/zstdport zstd <端口>`：设置 Zstd 对外端口

设置后会写入 `config/zstdnet-server.properties`。

`/zstdport` 是客户端指令，但只有管理员才能修改：

- 单机 / 局域网：本地房主且有命令权限时可修改
- 专用服：不会改到服务器配置，请直接改服务器上的 `config/zstdnet-server.properties`

### 5. zstdhud 状态面板

客户端还提供了一个简单的 HUD，用来在游戏内查看当前 zstd 连接状态和实时流量。
...
- `/zstdhud`：查看当前 HUD 开关状态
- `/zstdhud on`：显示 HUD
- `/zstdhud off`：关闭 HUD
- `/zstdhud toggle`：在开和关之间切换

HUD 会根据当前场景显示不同信息：

- 作为客户端连接远程 zstd 服务器时，会显示上下行线速、原始流量速率和累计传输量
- 作为局域网房主并启用了内置 zstd 入口时，会显示监听端口、压缩后带宽、原始带宽和累计传输量

如果你想确认当前是不是确实走了 zstd，打开 `zstdhud` 是最直接的办法。
...
5. Zstd 端口是否被其他程序占用。
6. 是否有其他模组拦截了登录或握手流程。
7. 你的局域网端口是否与game 端口是不是一致（使用/zstdport show查看）。
